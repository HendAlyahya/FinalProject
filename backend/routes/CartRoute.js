    ...
    router.post("/cart/post" ,async (req,res)=>{
        ...
        .then(async PUser =>{
            if(PUser.cart == undefined){
                Cart.create({cart: {products: req.body.product, qty:  req.body.qty, subtotal: req.body.qty*proudct1.price},total: req.body.qty*proudct1.price}).then(
                    async cart1 =>{
                          await User.findByIdAndUbdate(req.body.userId,{cart: cart1}).then(async(user)=>{
                    await user.save()
                    res.send(user)
                })
                    }
